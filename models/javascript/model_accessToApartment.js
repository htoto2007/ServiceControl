	...
	function AccessToApartments(){
		this.send = function(){
			
			var io = new IO;
			io.start({
				'idForm': "form#accessToApartment",					// Форма отправляемых данных
				'data': "",						// Если форма получения данных не определена в idForm, принемает массив отправляемых данных
				'url': "/controllers/php/controller_ajax_accessToApartments_send.php",						// Путь до обработчика данных
				...
				'successFunction': function(html){
					...
					if(arr['status'] == false){
						...
						io.notice("div.notice div.ok", "Изменения применены.");
						setTimeout(function(){
